...
move_template = new StatementStateTemplate("Move")
	.AddEnter(function() {
		...
		image_speed = 1;
		image_index = 0;
		EchoDebugWarn("This code will run for every state built from this template");
	})
	.AddUpdate(function() {
...
var _move = move_template.Build(id)
	.AddEnter(function() {
		EchoDebugWarn("This adds additional code for the state built from the template, that runs before the update code from the template.")
	}, eStatementBindMode.PREPEND)
	.AddEnter(function() {
		EchoDebugWarn("This adds additional code for the state built from the temple, that runs after the update code from the template.")
